...
class ChaseManager(Node):
    def __init__(self, executor):
        ...
        self.get_logger().info('Node Manager is ready.')
    ...
    def manage_nodes_callback(self, request, response):
        if request.data and not self.nodes_active:
            ...
            response.message = 'Nodes started successfully'
            ...
            self.nodes_active = False
            response.success = True
            response.message = 'Nodes stopped successfully'
        else:
            response.success = False
            response.message = 'No change in node status'
